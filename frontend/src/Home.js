...
function Home() {
  ...
  useEffect(() => {
    // Retrieve the last view from localStorage on component mount
  ...
  const handleCreateAudit = () => {
    ...
    localStorage.setItem('activeView', 'create'); // Store the view in localStorage
  };
  ...
  const handleEditAudit = () => {
    setActiveView('edit');
    localStorage.setItem('activeView', 'edit');
  };
  ...
  return (
    <div>
      ...
      {activeView === 'view' && <div>View Audits Component Goes Here</div>}
      {activeView === 'edit' && <div>Edit Audit Component (Under Construction)</div>}
    </div>
  );
}
