...
//       JSON.stringify({
//         ...formResponses,
//         [`image_${questionId}`]: updatedImages, // Update localStorage too
//       })
//     );
//     setSelectedImage(null); // Close the modal after removal
//   };
...
function QuestionBaseComponent({
  ...
  return (
    ...
    >
      ...
      {/* Image Upload Field */}
      <div className="flex-item image-upload" style={{ flexBasis: '15%', flexShrink: 0, position: 'relative' }}>
        <label htmlFor={`file-input-${uniqueQuestionId}`} className="file-label">
          <FaCamera style={{ fontSize: '1.5rem', cursor: 'pointer' }} />
        </label>
        <input
          ...
          {images[questionId] && Array.from(images[questionId]).map((file, index) => (
            <img
              ...
              style={{
                ...
                borderRadius: '5px',
                position: 'absolute',
                left: `${index * 20}px`, // Overlap with small horizontal offset
                zIndex: index,
                cursor: 'pointer',
              }}
              onClick={() => handleImageClick(file)} // Show full-size image on click
      ...
      {/* Duplicate and Remove Buttons */}
      <div className="flex-item action-buttons" style={{ display: 'flex', gap: '10px', marginLeft: '75px' }}>
        {!isDuplicate && (
          <button
            type="button"
            className="btn btn-secondary"
        ...
        <div className="image-modal" style={modalStyles}>
          <div style={modalContentStyles}>
            <img
              src={URL.createObjectURL(selectedImage)}
              alt="Selected"
              style={{ width: '80%', height: '70vh', objectFit: 'contain' }} // Force image to fit well
            />
            <button
              style={closeButtonStyles}
              onClick={() => setSelectedImage(null)} // Close the modal
